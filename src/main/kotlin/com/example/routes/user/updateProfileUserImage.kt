...
fun Route.updateProfileUserImage(repository: UserRepository) {
        ...
        try {
            ...
            multipart.forEachPart { partData ->
                when (partData) {
                    is PartData.FileItem -> {
                        ...
                        fileName = partData.saveImageProfile(PROFILE_PICTURE_USER_PATH)
                    }
                    else -> Unit
                }
            }
            val uri = "$PROFILE_PICTURES_USER$fileName"
            ...
            when (val result = repository.updateProfileImage(uri, call.userId.toInt())) {
                ...
                is Response.ErrorResponse -> {
                    File("$PROFILE_PICTURE_USER_PATH$fileName").delete()
                    call.respond(
                        result.statusCode, UserResponse(
                            status = ERROR,
                            message = result.message,
...
private suspend fun isTheSameImage(imageURL: String?, repository: UserRepository) {
    imageURL?.let {
        if (repository.isTheSameImage(imageURL)) {
            File("$PROFILE_PICTURE_USER_PATH$imageURL").delete()
        }
    }
}